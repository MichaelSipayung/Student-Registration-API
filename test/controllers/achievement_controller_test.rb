...
class AchievementControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should update achievement' do
    ...
    body = JSON.parse(response.body)
    @token = body['token']
    patch achievement_path(achievements(:one)), params: { achievement: {
    ...
    patch achievement_path(achievements(:three)), params: { achievement: {
  ...
  test 'should show achievement' do
    ...
    body = JSON.parse(response.body)
    @token = body['token']
    get achievement_url(achievements(:one)),
        headers: {'Authorization'=>"Bearer #{@token}"}, as: :json
    assert_response :success
  end
end
