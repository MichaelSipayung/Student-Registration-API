...
class UtbkScoresControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create utbk score' do
    assert_difference 'UtbkScore.count' do
      post api_v1_utbk_scores_url, params: {
  ...
  test 'should update utbk score' do
    ...
    body = JSON.parse(response.body)
    @token = body['token']
    patch api_v1_utbk_score_path(utbk_scores(:one)), params: {
    ...
    assert_equal 100.5,
                 utbk_scores(:one).reload.nilai_kemampuan_memahami_bacaan_dan_menulis
  ...
  test 'should show utbk score' do
    ...
    body = JSON.parse(response.body)
    @token = body['token']
    get api_v1_utbk_score_url(utbk_scores(:one)),
        headers: {'Authorization'=>"Bearer  #{@token}"}, as: :json
    assert_response :success
  end
end
