...
class ParentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create parents data' do
    assert_difference 'Parent.count' do
      post api_v1_parents_path, params: { parent: {
        nama_ayah: 'luhut', nama_ibu: 'dinar', nik_ayah: 1_234_567_890_121_345,
        nik_ibu: 1_234_567_890_123_415,
  ...
  test 'should update parents data' do
    ...
    body = JSON.parse(response.body)
    @token = body['token']
    patch api_v1_parent_path(parents(:one)), params: { parent: {
  ...
  test 'should show parents information' do
    ...
    body = JSON.parse(response.body)
    @token = body['token']
    get api_v1_parent_url(parents(:one)), headers: {'Authorization'=>"Bearer #{@token}"},
        as: :json
    assert_response :success
  end
